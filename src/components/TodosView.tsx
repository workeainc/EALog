import { AlertCircle, Bell, CalendarDays, Check, CheckCircle2, ChevronLeft, ChevronRight, ListTodo, Move, Pencil, Plus, Search, StickyNote, Trash2, X } from "lucide-react";
...
export default function TodosView({ todos, projects, defaultProjectId, onCreate, onToggle, onMove, onUpdate, onDelete, onOpenNote }: Props) {
  ...
  return <section className="task-target" aria-label="Tasks workspace">
    ...
    <div className="task-target-grid"><article className="task-target-main"><h3>{panelTitle} <span>({displayedTodos.length})</span></h3>{displayedTodos.length ? <div>{displayedTodos.map((todo) => <TargetTaskCard key={todo.id} todo={todo} project={projectFor(todo.projectId)} onToggle={onToggle} onMove={onMove} onDelete={onDelete} onEdit={setEditingTodo} onOpenNote={onOpenNote} canMove={todo.status === "open" && todo.plannedDateString !== today} />)}</div> : <div className="task-target-empty"><ListTodo size={28} /><b>No tasks here</b><p>Add the work you want to finish.</p></div>}</article><aside className="task-target-side"><TargetSidePanel title="Overdue" todos={overdue.filter(matchesSearch)} projects={projects} icon={<AlertCircle size={17} />} tone="overdue" onToggle={onToggle} onMove={onMove} onDelete={onDelete} onEdit={setEditingTodo} /><TargetSidePanel title="Recently completed" todos={recentlyCompleted.filter(matchesSearch).slice(0, 5)} projects={projects} icon={<CheckCircle2 size={17} />} tone="completed" onToggle={onToggle} onMove={onMove} onDelete={onDelete} onEdit={setEditingTodo} /></aside><aside className="task-target-calendar"><TaskCalendar todos={todos} month={calendarMonth} selectedDate={date} onPrevious={() => setCalendarMonth((current) => new Date(current.getFullYear(), current.getMonth() - 1, 1))} onNext={() => setCalendarMonth((current) => new Date(current.getFullYear(), current.getMonth() + 1, 1))} onSelect={selectDate} /><p><CalendarDays size={15} /> Click a date to view its tasks.</p></aside></div>
...
function TargetTaskCard({ todo, project, onToggle, onMove, onDelete, onEdit, onOpenNote, canMove }: { todo: Todo; project?: Project; onToggle: Props["onToggle"]; onMove: Props["onMove"]; onDelete: Props["onDelete"]; onEdit: (todo: Todo) => void; onOpenNote?: (noteId: string) => void; canMove: boolean }) {
