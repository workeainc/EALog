...
export const isVaultRecord = (value: unknown): value is VaultRecord => { const record = value as Partial<VaultRecord>; const validWorkbook = record.kind !== "workbook" || (typeof record.title === "string" && Array.isArray(record.sheets) && record.sheets.every((sheet) => sheet && typeof sheet.id === "string" && typeof sheet.name === "string" && Array.isArray(sheet.headers) && Array.isArray(sheet.rows))); return Boolean(record && typeof record.id === "string" && typeof record.category === "string" && record.fields && typeof record.fields === "object" && (record.projectId === null || typeof record.projectId === "string") && Number.isFinite(record.createdAt) && Number.isFinite(record.updatedAt) && validWorkbook); };
