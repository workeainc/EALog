        ...
        (snapshot) => {
            const projects = snapshot.docs.map((item) => {
                ...
                if (
                    ...
                    throw new Error(
                        `Project "${item.id}" has invalid configuration.`,
                    );
                }
                return {
                    id: item.id,
...
async function applyCommand(
    ...
    await runTransaction(db, async (tx) => {
        ...
        if (command.type === "start") {
            if (remote)
                throw new Error(
                    "Conflict: another active session already exists.",
                );
        ...
        if (command.type === "stop") {
            ...
            if (remoteRevision !== payload.baseRevision)
                throw new Error(
                    "Conflict: remote session changed on another device.",
                );
...
export async function updateProject(
    ...
    await setDoc(doc(projectsRef(uid), id), data, { merge: true });
}
